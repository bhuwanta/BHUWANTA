...
export default async function ShabadOpenPlotsPage() {
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-[#002935] pt-32 sm:pt-40 pb-16 sm:pb-20">
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#B69A4E]/10 rounded-full blur-[100px]" />
        <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <p className="flex items-center gap-2 text-[#d4b872] text-sm font-semibold mb-5"><MapPin className="w-4 h-4" /> Shabad · Southwest Hyderabad</p>
              <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight leading-tight mb-5">Open Plots in Shabad.<br /><span className="text-[#d4b872]">Explore Vian Valley.</span></h1>
              ...
              <div className="mt-7 border-l-2 border-[#d4b872] pl-5">
                <p className="text-white/70 text-sm">Plots from</p>
                <p className="text-3xl font-bold text-[#d4b872]">₹30,999 <span className="text-base font-normal text-white/80">per sq. yd.</span></p>
                <p className="text-xs text-white/65 mt-2">Request a plot-specific quote and full cost breakdown.</p>
              </div>
              <div className="mt-8 flex flex-col sm:flex-row gap-3">
                <Link href="#book-visit" className="inline-flex items-center justify-center gap-2 px-6 py-4 bg-[#d4b872] text-[#002935] font-bold rounded-xl hover:bg-[#e2ca91] transition-colors">Get Price &amp; Plot Details <ArrowRight className="w-4 h-4" /></Link>
      ...
      <section className="py-14 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 grid md:grid-cols-3 gap-7">
          ...
          ].map(item => <div key={item.title} className="rounded-xl border border-[#e8ecf2] p-6"><item.icon className="w-6 h-6 text-[#b59853] mb-4" /><h2 className="text-lg font-bold text-[#0f1d33] mb-2">{item.title}</h2><p className="text-sm leading-relaxed text-[#5a6a82]">{item.text}</p></div>)}
        </div>
      </section>
      <section className="py-14 bg-[#f7f8fa]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#0f1d33] mb-5">Shabad’s wider industrial corridor</h2>
          <p className="text-[#5a6a82] leading-relaxed mb-4">Shabad buyers often compare access to Chandanvelly, Hayathabad and Seetharampur. These are distinct locations: check the route from the particular plot rather than relying on a regional map or a headline travel time.</p>
          <p className="text-[#5a6a82] leading-relaxed mb-4">Olectra reported the start of commercial operations at its Seetharampur EV facility on 31 December 2025. Its first phase has an annual production capacity of 2,500 buses per shift. <a href="https://olectra.com/wp-content/uploads/23.-01.01.2026.pdf" target="_blank" rel="noopener noreferrer" className="underline text-[#1e3a5f]">Read Olectra’s announcement</a>.</p>
          <p className="text-[#5a6a82] leading-relaxed">Consider both existing activity and announced plans when comparing locations. Future development does not guarantee a particular resale price or completion date.</p>
          <Link href="/projects/vian-vally" className="inline-flex items-center gap-2 font-semibold text-[#1e3a5f] mt-6">View Vian Valley project details <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </section>
      <section id="book-visit" className="py-16 bg-white scroll-mt-28">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 grid lg:grid-cols-[0.85fr_1.15fr] gap-10">
          <div>
            <p className="text-sm font-bold uppercase tracking-widest text-[#b59853] mb-3">Your next step</p>
            <h2 className="text-3xl font-bold text-[#0f1d33] mb-5">Find the plot that fits your plans.</h2>
            <p className="text-[#5a6a82] leading-relaxed">Share your requirements to request available plot sizes, a written cost breakdown and a site visit. Whether you are comparing options or ready to visit, our team can help you take the next step.</p>
            <p className="text-sm text-[#5a6a82] mt-5">You can include your budget, preferred plot size and buying timeline in the optional message.</p>
          </div>
          <div className="p-6 sm:p-8 rounded-2xl border border-[#e8ecf2] bg-[#f7f8fa]"><ContactForm projectsList={projects} locationNames={['Shabad']} initialProject={projects[0].name} compact /></div>
        </div>
      </section>
      <section className="py-16 bg-[#f7f8fa]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl font-bold text-[#0f1d33] mb-8">Questions about Shabad plots</h2>
          <div className="space-y-6">{faqs.map(faq => <div key={faq.question}><h3 className="font-bold text-[#0f1d33] mb-2">{faq.question}</h3><p className="text-[#5a6a82] leading-relaxed">{faq.answer}</p></div>)}</div>
          <div className="mt-10 pt-6 border-t border-[#e8ecf2] flex flex-wrap gap-5 text-sm font-semibold text-[#1e3a5f]"><Link href="/blog/open-plots-shabad-hyderabad-hmda-approved-guide">Shabad buyer’s guide</Link><Link href="/blog/shabad-vs-shadnagar-investment-comparison">Shabad vs Shadnagar</Link><Link href="/resources/hyderabad-plot-buyer-legal-checklist">Plot document checklist</Link></div>
        </div>
      </section>
    </>
  )
